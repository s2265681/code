...
import _ from 'lodash'
  ...
  interface CommissionProps {
     ...
     * e,itemId,_newData
    */
    onChange?:(e:React.MouseEvent,item:object,index:number)=>void
  }
  ...
  const Commission:React.FC<CommissionProps>=(props)=>{
     ...
     // 更改代办事项状态
     const radioChange=(e: React.MouseEvent<Element, MouseEvent>,itemId: object,index: number)=>{
        const _newData = changeEveryVal(itemId,_.cloneDeep(_dataSource),'RadioChange')
        onChange&&onChange(e,itemId,_newData)
        setDate(_newData)
     }
     // 删除代办项
     const deleteCurrentItem=(e: React.MouseEvent<Element, MouseEvent>,itemId: object,index: number)=>{
     ...
     // 操作数组
     const changeEveryVal=(id: object,arr:any,type:string)=>{
           ...
           return dataSource.map((item,index)=>{
                ...
                return(
                    <div className="info-wrapper" key={key}>
                         ...
                         >
                            {isExpand&& <Icon icon="chevron-down" size="1x"></Icon>}
                            {(isHandle&&currentId===item.id)&& <>
                                <input type="radio" checked={item.isDone} onClick={e=>radioChange&&radioChange(e,item.id,index)}/>
